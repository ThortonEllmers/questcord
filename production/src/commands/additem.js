...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('additem')
    .setDescription('Give an item to a user (Staff/Developer only)')
    .addUserOption(o=>o.setName('user').setDescription('Target user').setRequired(true))
    .addStringOption(o=>o.setName('item').setDescription('Item to give').setAutocomplete(true).setRequired(true))
  ...
  async execute(interaction){
    ...
    if (!inv) db.prepare('INSERT INTO inventory(userId,itemId,qty) VALUES(?,?,?)').run(target.id, item.id, amount);
    else db.prepare('UPDATE inventory SET qty=qty+? WHERE userId=? AND itemId=?').run(amount, target.id, item.id);
    logger.info('admin_additem: %s added %s x%s to %s', interaction.user.id, item.id, amount, target.id);
    return interaction.reply(`${userPrefix} Granted **${amount}x ${item.name}** to <@${target.id}>.`);
  }
};
